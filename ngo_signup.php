...
if (isset($_POST['signup'])) {
    ...
    if ($password == $cpassword) {
        if (strlen($p) > 5) {
            $query = "SELECT * FROM ngo WHERE username = '$username' OR email = '$email'";
            $query_run = mysqli_query($conn, $query);

            if ($query_run && mysqli_num_rows($query_run) == 0) {
                $query2 = "INSERT INTO ngo(username, email, `password`, code) VALUES ('$username', '$email', '$password', '$code')";
                ...
                if ($query_run2) {
                    ...
                    try {
                        ...
                        $mail->Body = "Welcome, $username!<br>Click <a href='http://localhost/wecare/ngo_verify.php?code=$code'>here</a> to verify your email.";
